                    ...
                    <div class="row white_shd full margin_bottom_30 mt-4 pb-5">
                        ...
                        <div class="col-lg-6 mt-3">
                            ...
                            <input type="text" required class="form-control mb-2 mr-sm-2"
                                   placeholder="reference of machine"
                                   id="reference">
                        </div>
                        <div class="col-lg-6 mt-3">
                        ...
                        <div class="col-lg-6 mt-3">
                            <label for="prix" class="mr-sm-2">Prix :</label>
                            <input type="number" required class="form-control mb-2 mr-sm-2"
                                   placeholder="Machine price"
                                   id="prix">
                        </div>
                        <div class="col-lg-6 mt-3">
                            <label for="date_achat" class="mr-sm-2">Date d'achat :</label>
                            <input type="date" required class="form-control mb-2 mr-sm-2"
...
<script>
    $(document).ready(function () {
        ...
        $('#create_button').click(function () {
            ...
            let salle_id = $('#salle_id').val()

            if (reference === '' || prix === '' || date_achat === '' || marque === '' || salle_id === '') {
                $('#error_message').text('Fill the inputs first')
            } else {
                $('#error_message').text('')
                $.ajax({
                    url: "<%=request.getContextPath()%>/machines?op=create",
                    data: {
                        'reference': reference,
                        'prix': prix,
                    ...
                    success: function (data) {
                        ...
                        if (data === true) {
                            $('#sucessmodal').modal('show')
                            $('#success_message').text('The machine created success')
                            load()
                        }
                    },
                    error: function (e) {
        ...
        $('#update_button').click(function () {
            let id = $('#update_machine_id').val()
            let salle = $('#salle_id').val()
            let reference = $('#reference').val()
            let marque = $('#marque').val()
            let date_achat = $('#date_achat').val()
            let prix = $('#prix').val()
            if (salle === '' || reference === '' || marque === '' || date_achat === '' || prix === '') {
                ...
                $('#error_message').text('')
                $.ajax({
                    url: "<%=request.getContextPath()%>/machines?op=update",
                    data: {
                        'id': id,
                        'salle': salle,
                        'reference': reference,
                    ...
                    success: function (data) {
                        ...
                        if (data === true) {
                            $('#sucessmodal').modal('show')
                            $('#success_message').text('The machine updated success')
                            load()
                        }
                    },
                    error: function (e) {
