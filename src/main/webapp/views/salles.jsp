                    ...
                    <div class="row white_shd full margin_bottom_30 mt-4 pb-5">
                        ...
                        <div class="col-lg-6 mt-3">
                            ...
                            <input type="text" required class="form-control mb-2 mr-sm-2"
                                   placeholder="code of salle"
                                   id="code">
                        </div>
                        <div class="col-lg-6 mt-3">
                            <label for="type" class="mr-sm-2">Type</label>
                            <select type="text" class="form-control mb-2 mr-sm-2" id="type">
                                <option value="Simple salle">Simple salle</option>
                                <option value="Computer salle">Computer salle</option>
                                <option value="Network salle">Network salle</option>
                            </select>
                        </div>
                        <div class="col-12 mt-3">
                            <p class="mb-3 text-danger" id="error_message"></p>
                            <button class="main_bt mr-5" id="create_button">Create</button>
                            <button class="main_bt" id="update_button"
                                    style="background-color: #009688 !important;display: none">Update
                            </button>
                        </div>
                    </div>
                    <input type="hidden" id="update_salle_id"/>
                    <div class="row">
                        <div class="white_shd full margin_bottom_30">
                            <div class="full graph_head">
                                <div class="heading1 margin_0">
                                    <h2>Liste of salle</h2>
                                </div>
                            </div>
                                ...
                                <div class="table-responsive-sm">
                                    <table class="table">
                                        ...
                                        <tr>
                                            ...
                                            <th><input type="text" class="form-control find findby_code" target="code">
                                            </th>
                                            <th><input type="text" class="form-control find findby_type" target="type">
                                            </th>
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '.delete_button', function () {
            $('#delete_input').val($(this).attr('salle_id'))
            $('#deletemodal').modal('show')
        })
        $('#delete_btton_model').click(function () {
            $.ajax({
                url: "<%=request.getContextPath()%>/salles?op=delete",
                data: {
                    'id': $('#delete_input').val(),
                },
                type: 'POST',
                success: function (data) {
                    ...
                    if (data === true) {
                        ...
                        $('#success_message').text('The salle deleted success')
        ...
        $('#create_button').click(function () {
            ...
            if (code === '' || type === '') {
                ...
                $('#error_message').text('')
                $.ajax({
                    url: "<%=request.getContextPath()%>/salles?op=create",
                    data: {
                        'code': code,
                        'type': type,
                    },
                    ...
                    success: function (data) {
                        ...
                        if (data === true) {
                            $('#sucessmodal').modal('show')
                            $('#success_message').text('The salle created success')
                            load()
                        }
                    },
                    error: function (e) {
        ...
        $(document).on('click', '.table_row', function (e) {
            ...
            $('#type').val($(this).children('td:nth-child(3)').text())
            $('#update_salle_id').val($(this).children('td:nth-child(1)').text())
            $('#update_button').show()

        })
        $(document).on('click', function (e) {
            if ($(e.target).closest(".table_row").length === 0 &&
                $(e.target).closest("#code").length === 0 && $(e.target).closest("#type").length === 0) {
                $('#update_salle_id').val("")
                $('#update_button').hide()
            }
        });
        ...
        $('#update_button').click(function () {
            ...
            if (code === '' || type === '') {
                ...
                $('#error_message').text('')
                $.ajax({
                    url: "<%=request.getContextPath()%>/salles?op=update",
                    data: {
                        'id': id,
                        'code': code,
                        'type': type,
                    ...
                    success: function (data) {
                        ...
                        if (data === true) {
                            $('#sucessmodal').modal('show')
                            $('#success_message').text('The salle updated success')
                            load()
                        }
                    },
                    error: function (e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        html += '<tr class="table_row" salle_id="' + data[i].id + '"><td>' + data[i].id + "</td><td>" + data[i].code + "</td><td>" + data[i].type +
                            "</td><td>" + data[i].created_at + "</td>" + '<td><button class="btn btn-danger delete_button" salle_id="' + data[i].id + '">Delete</button></td>';
                    }
        ...
        function load() {
            $.ajax({
                url: "<%=request.getContextPath()%>/salles?op=getdata",
                data: {},
                type: 'POST',
                success: function (data) {
                    html = "";
